 ...
 */
class LabelPage implements Printable {
    private PDFFile labelPDF;

    public LabelPage(PDFFile labelPDF) {
    ...
    public int print(Graphics g, PageFormat format, int index)
        ...
        if ((pagenum >= 1) && (pagenum <= labelPDF.getNumPages())) {

            // fit the PDFPage into the printing area
            Graphics2D g2 = (Graphics2D) g;
            PDFPage page = labelPDF.getPage(pagenum);
